...
import AnnulerAc from './AnnulerValidationAc';
...
class MandatViewsAllAC extends React.Component {
    ...
    componentDidMount(){
        fetch(`http://localhost:8000/mandatement/MandatementAc/${this.props.match.params.id_Ac}`)
            .then(res => res.json())
    ...
    render() {
        return (
            ...
            {!this.state.liste.valid_directeur && <h5 className="section-title text-center"><ValiderAc id={this.props.match.params.id_Ac} /> </h5>}
            {this.state.liste.valid_directeur && <h5 className="section-title text-center"><AnnulerAc id={this.props.match.params.id_Ac} /> </h5>}
            </div>
                <div className="row">
