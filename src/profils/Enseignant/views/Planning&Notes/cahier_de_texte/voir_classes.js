...
function ClassesList(props) {
    ...
    const [role] = React.useState(() => {
        ...
        props.auth.user.CurrentRoles.map(role => {
            ...
            else if (role.role_type === "RESPONSABLE_PEDAGOGIQUE"){
                history.replace(`/enseignant/voir-progressions-classe/${role.classe}/${role.departement}`);
            }
    ...
    React.useEffect(async () => {
        ...
        await loadClasses(role.departement).then(classes => {
            ...
            console.log(e)
            alert("Une erreur est survenue lors du chargment des données")
            setLoadingState(false);
        }
    }, [])
    return (
                    ...
                    <Grid container spacing={3}>
                        {classes.map((classe, index) =>
                            <Grid item xs={3} key={index}>
                                <Link to={`/enseignant/voir-progressions-classe/${classe.niveau}/${classe.departement}`}>
