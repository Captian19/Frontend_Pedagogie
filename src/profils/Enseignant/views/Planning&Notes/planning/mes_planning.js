...
import { loadPlanningsProf } from "../../../../../actions/Planning&Notes/planning_services";
...
function VoirMesPlanning(props) {
    ...
    useEffect(async () => {
        try {
            await loadPlanningsProf(role.id, role.annee).then(plannings => {
        ...
        loading ? <LoadingSpinner loading={loading} />
            ...
            plannings.length == 0 ?
                ...
                plannings.map(planning => {
                    return (
                        <VoirPlanning planning={planning} enseignant={true} is_res_peda={role.is_res_peda} niveau={role.niveau ?? null} departement={role.departement ?? null} />
                    )
                })
    )
}
