...
const ListClasse = (props) => {
    ...
    const onSubmit = (data) => {
        getListByClasse(data.classe, role.role_type === "ASSISTANT_CHEF_DEPARTEMENT" ? role.departement : data.departement, data.annee);
        console.log(data);
    }
                        ...
                        <CRow>
                            <CCol sm="12" lg="15">
                                <CDataTable
                                    ...
                                    paginationfooter
                                    clickableRows
                                    onRowClick={(item) => history.push(`/${role.role_type === "ASSISTANT_CHEF_DEPARTEMENT" ? "assistant-departement" :  "scolarite" }/voir-resultats-annuels-eleve/${item.id}/${item.user.id}`)}
                                    hover
                                    sorter
                                    hover
                                    pagination
